# 📊 LRM-Eval
🏠[Home](https://github.com/flageval-baai/LRM-Eval) | 🤗[Data](https://huggingface.co/datasets/BAAI/ROME) | 🤗[Model Response](https://huggingface.co/datasets/BAAI/ROME-V-evaluated) | 💻[Github Code](https://github.com/flageval-baai/ROME-evaluation) | 📄[Paper](https://arxiv.org/) | *Current Version: v1.0*
...
## 📰 News
**[07/23/2024]** 🚀 First release of Rome.
We released our [leaderboard](https://huggingface.co/datasets/princeton-nlp/CharXiv/tree/main/existing_evaluations) on **30+ LLMs and MLLMs** that we have tested so far.
We also released all model responses across 4 rounds of evaluations([Model response](https://huggingface.co/datasets/princeton-nlp/CharXiv/tree/main/existing_evaluations)).
...
### Text tasks
Textual problems includes the following subtasks:
...
Visual tasks includes the following 8 subtasks in total(281 high quality questions), each sample is carefully checked and ensured that the images are necessary to correctly answer the questions:
* Academic
...
## 📐 Evaluation Script Usage (src/evaluate.py)
...
- number_matching: match the model's answer with the number (with 10% relative error for float and 1e-3 absolute error for integer)
- location_matching: match the model's answer with the location(fine-grained and coarse-grained location matching)

### How to use the evaluator script

Convert your evaluation into specific format as listed in [Inputs and formats](#inputs-and-formats). Then execute the following command:
...
- `--use_llm_evaluator`: enable LLM-based semantic grading (otherwise rule-based only).
- `--llm_model`: model name for LLM grading. Default reads from `OPENAI_MODEL` or uses `gpt-4.1-mini`.
- `--output`: output path for scores. Default: `scores.json` next to the predictions file.
...
- Annotations (JSON or JSONL):
  ...
  - Each item must include at least:
    - `question_id`: a identifier for the question
    - `evaluator`: one of: `key_items_matching`, `choices_matching`, `ordered_list_matching`, `number_matching`, `location_matching`, `interval_matching`, `multi_interval_matching`
    - `evaluator_kwargs`: evaluator-specific kwargs (see below)
